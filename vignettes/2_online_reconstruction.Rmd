...
If you look into the [*Details* of the reference of `reconstruct()`](../reference/reconstruct.html#details), you will see that the `model` argument can be set to other character strings besides `"MERDITH2021".` These indicate other models that are accessible via the GPLates Web Service. For instance, if you want to execute the same calculations to [reconstruct the position position of the plates at 200Ma](#reconstructing-plates), with the Seton et al. 2012 model, all you have to do is set `model="SETON2012"`:
