...
expect_equal(length(sol), length(ages))
expect_equal(as.numeric(names(sol)), ages)
...
# multiple raster from 0 to past
ages <- c(60,100)
expect_silent(sol <- reconstruct(smallDouble, age=ages, model="MERDITH2021"))
expect_equivalent(class(sol), "list")
expect_equal(length(sol), length(ages))
expect_equal(as.numeric(names(sol)), ages)
expect_true(inherits(sol[[1]], "SpatRaster"))
...
# multiple raster from older to younger
ages <- c(30,15)
expect_silent(sol <- reconstruct(pastDouble, from=60, age=c(30, 15), model="MERDITH2021"))
expect_equivalent(class(sol), "list")
expect_equal(length(sol), length(ages))
expect_equal(as.numeric(names(sol)), ages)
expect_true(inherits(sol[[1]], "SpatRaster"))
...
# multiple raster from younger to older
ages <- c(120,150)
expect_silent(sol <- reconstruct(pastDouble, from=60, age=c(120,150), model="MERDITH2021"))
expect_equivalent(class(sol), "list")
expect_equal(length(sol), length(ages))
expect_equal(as.numeric(names(sol)), ages)
expect_true(inherits(sol[[1]], "SpatRaster"))
