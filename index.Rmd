...
- Major update to v0.6 to accommodate changes to the GPlates Web Service (v1.0) 
- New plotting functions to make more comprehensive paleogeographic maps (subduction zones, see below) 
...
From version 0.6.0, the package supports topological plate boundary export from the [GWS](https://gws.gplates.org/), demonstrated here with the [`CAO2024`](https://doi.org/10.1016/j.gsf.2024.101922) model:

```{r subduction, include=FALSE}
library(rgplates)
...
png("man/figures/subduction_example.png", width=1400,height=600)
	# plot
	plot(boundaries[, "type"], lwd=2, reset=FALSE, main="", key.pos=4)
